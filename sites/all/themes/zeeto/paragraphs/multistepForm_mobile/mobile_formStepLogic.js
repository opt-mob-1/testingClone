...
/**
 * Cloned from multistepForm/multi_step2.js
...
function initializeFormStepLogic() {
  var formLogic = {
    formSteps: [{
      ...
      inputs: ['#edit-gender-f', '#edit-gender-m', '#ga_client_id'],
      stepLoadedCustom: function stepLoadedCustom(attachListeners) {
        /* begin custom functionality */
        this.gaClientId();
      ...
      stepSubmittedCustom: function stepSubmittedCustom() {
        ...
        /* end custom functionality */
        dataLayer.push({ 'event': 'progression', 'phase': 'completedSignup' });
        ...
        if (variationNextPage) {
          ...
          window.location = '/z/072017/survey?uf=' + uf;
        }
      }
    }],

    // Adds the value of the utm_content param to the hidden input
    addSampleImage: function addSampleImage() {
    ...
    // sets value of hidden input
    gaClientId: function gaClientId() {
      if (window.ga) {
        window.ga(function () {
